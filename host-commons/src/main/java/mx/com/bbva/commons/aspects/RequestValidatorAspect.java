package mx.com.bbva.commons.aspects;
...
import mx.com.bbva.commons.exceptions.ForbiddenException;
import mx.com.bbva.commons.exceptions.NotFoundException;
import mx.com.bbva.commons.exceptions.ParametersException;
import mx.com.bbva.commons.exceptions.UnauthorizedException;
import mx.com.bbva.commons.to.ErrorTO;
...
public class RequestValidatorAspect {
	...
	@Around(value = "execution(* mx.com.bbva.web.rest.*.*(..))  && args(..)")
	public ResponseEntity execute(ProceedingJoinPoint joinPoint) {
		...
		try {
			...
			String parameters = Arrays.stream(joinPoint.getArgs()).map(Object::toString)
			...
			LOG.info("**** [FINALIZA METODO] - Tiempo [{} segundos o {} millisegundos] ****", secondsElapsed,
					millisecondsElapsed);
			return result;
